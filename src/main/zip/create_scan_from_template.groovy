...
String scanTemplateId = props.notNull("scanTemplateId")
...
println "Scan Template Id: ${scanTemplateId}"
...
try {
    ...
    def scanTemplateName = wieClient.getScanTemplateNameById(scanTemplateId)
    println "Found template name: \"${scanTemplateName}"
    def wieConsoleUrl = "${wieServerUrl}/WebConsole/ApplicationDetails.aspx?SiteID=${scanSiteId}"

    println("Executing scan \"${scanName}\" using template \"${scanTemplateName}\"")
    println("See: ${wieConsoleUrl}")

    long startTime = new Date().getTime();
    wieClient.debug("Start UNIX time is: ${startTime}")
    scanId = wieClient.createScanFromTemplate(scanName, scanSiteId, scanPolicyId, scanSensorId, scanTemplateId, Integer.valueOf(scanPriority))
